...
from puncc import conformalization, calibration, prediction, splitting
...
def test_split_cp(diabetes_data, alpha, random_state):
    ...
    ## Calibration
    calibrator = calibration.MeanCalibrator(w_estimator=w_estimator)
    ## Splitter
    ...
    # the risk alpha
    y_pred, y_pred_lower, y_pred_upper, _ = w_split_cp.predict(
        X_test, alpha=alpha
    )

...
into [`deel.puncc.api.conformalization.ConformalPredictor`][deel.puncc.api.conformalization.ConformalPredictor]:
...
[`deel.puncc.api.conformalization.ConformalPredictor`][deel.puncc.api.conformalization.ConformalPredictor] is the canvas of conformal prediction procedures.
...
[`deel.puncc.api.conformalization.ConformalPredictor`][deel.puncc.api.conformalization.ConformalPredictor] implements two methods:

* A `fit` method that fits the predictor model and computes nonconformity scores accordingly to the calibrator and to the data split strategy provided by the splitter
...
The [`deel.puncc.api.prediction.BasePredictor`][deel.puncc.api.prediction.BasePredictor] and [`deel.puncc.api.prediction.DualPredictor`][deel.puncc.api.prediction.DualPredictor] classes are wrappers of ML/DL models
...
The constructor of [`deel.puncc.api.prediction.BasePredictor`][deel.puncc.api.prediction.BasePredictor] takes in the model to be wrapped, a flag to inform if the model is already trained
and compilation keyword arguments if the underlying model needs to be compiled (such as in keras).

The constructor of [`deel.puncc.api.prediction.DualPredictor`][deel.puncc.api.prediction.DualPredictor] is conceptually similar but take as arguments
...
Note that the output `y_pred` of the `predict` method are a collection of couples,
where the first (resp. second) axis is associated to the output of the first (resp. second) model.
Specifically, [`deel.puncc.api.prediction.MeanVarPredictor`][deel.puncc.api.prediction.MeanVarPredictor] is a subclass of [`deel.puncc.api.prediction.DualPredictor`][deel.puncc.api.prediction.DualPredictor] that
...
But if you have a special need, you can subclass [`deel.puncc.api.prediction.BasePredictor`][deel.puncc.api.prediction.BasePredictor] or [`deel.puncc.api.prediction.DualPredictor`][deel.puncc.api.prediction.DualPredictor] or
...
from sklearn. The procedure [RAPS](theory_overview.md#theory-raps) to conformalize the classifier requires
a `predict` method that outputs the estimated probability of each class. This is not the case
...
on the calibration set and to compute the prediction sets. At the constructor [`deel.puncc.api.calibration.BaseCalibrator`][deel.puncc.api.calibration.BaseCalibrator],
...
[`deel.puncc.api.calibration.BaseCalibrator.fit`][deel.puncc.api.calibration.BaseCalibrator.fit] on the calibration dataset. Based on the estimated quantiles of nonconformity scores,
the method [`deel.puncc.api.calibration.BaseCalibrator.calibrate`][deel.puncc.api.calibration.BaseCalibrator.calibrate] enables to **construct** and/or **calibrate** prediction sets.
...
`deel.puncc.api.nonconformity_scores.mad` and [`deel.puncc.api.prediction_sets.constant_interval`][deel.puncc.api.prediction_sets.constant_interval], respectively:
...
The two plans are implemented in the [`deel.puncc.api.splitting`](splitting.md) module,
and are agnostic to the data structure (which can be ndarrays, tensors and dataframes):

- [`deel.puncc.api.splitting.RandomSplitter`][deel.puncc.api.splitting.RandomSplitter]: random assignment of samples in $D_{fit}$ and $D_{calib}$
- [`deel.puncc.api.splitting.KFoldSplitter`][deel.puncc.api.splitting.KFoldSplitter]: random assignment of samples into K disjoint folds. Note that if K equals the size of training set, the split is identified with the leave-one-out strategy

Additionnaly, if the user already implemented a split plan, the obtained data asignement
is wrapped in [`deel.puncc.api.splitting.IdSplitter`][deel.puncc.api.splitting.IdSplitter] to produce iterables.

These methods produce **iterables** that are used by the [`ConformalPredictor`][deel.puncc.api.conformalization.ConformalPredictor] instance.

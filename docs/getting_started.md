...
[`deel.puncc.api.prediction`][deel.puncc.api.prediction].
...
[XGBoost](https://xgboost.readthedocs.io/en/stable/). **An example of**
[conformal classification](#classification-example) **with keras models
is provided later in this page**.
For more information about model wrappers and supported ML/DL libraries,
checkout [the documentation](prediction.md).

For a linear regression from scikit-learn, we use
[`deel.puncc.api.prediction.BasePredictor`][deel.puncc.api.prediction.BasePredictor] as follows:
...
[`deel.puncc.regression.SplitCP`][deel.puncc.regression.SplitCP]. Other methods are presented
[here](regression.md).
...
The library provides several metrics in [`deel.puncc.metrics`][deel.puncc.metrics] to evaluate
...
In addition, `puncc` provides plotting tools in [`deel.puncc.plotting`][deel.puncc.plotting]
...
<p align="center">
  <img src="../assets/results_quickstart_split_cp_pi.png" alt="90%-prediction interval with the split conformal prediction method" width="600" height="300">
</p>
<p align="center"><em>90%-prediction interval with the split conformal prediction method</em></p>

## 📊 Conformal Classification

Let's tackle the classic problem of
[MNIST handwritten digits](https://en.wikipedia.org/wiki/MNIST_database)
...
MNIST dataset contains a large number of $28 \times 28$ digit images to which are associated digit labels.
As the data generating process is considered i.i.d (check [this post](https://newsletter.altdeep.ai/p/the-story-of-mnist-and-the-perils)),
...
For the convnet above, we use [`deel.puncc.api.prediction.BasePredictor`][deel.puncc.api.prediction.BasePredictor] as wrapper.
...
<a id="classification-example"></a>

The [RAPS](theory_overview.md#theory-raps) procedure is chosen to conformalize our convnet classifier.
Such algorithm has two hyparameters $\lambda$ and $k_{reg}$ that encourage smaller prediction sets.

To start off gently, we will ignore the regularization term ($\lambda = 0$), which simply turns the procedure into [APS](theory_overview.md#theory-aps):
...
<p align="center">
  <img src="../assets/results_quickstart_aps_mnist.png" alt="results quickstart aps mnist" width="300" height="300">
</p>
...
remains below the user-specified threshold $\alpha$.
...
[`deel.puncc.api.prediction`][deel.puncc.api.prediction]. For more information about model wrappers
and supported ML/DL libraries, checkout [the documentation](prediction.md).
...
The [`deel.puncc.anomaly_detection.SplitCAD`][deel.puncc.anomaly_detection.SplitCAD] wrapper is used to train
...
<p align="center">
  <img src="../assets/results_quickstart_split_cad_if.png" alt="results quickstart split cad if" width="600" height="300">
</p>
